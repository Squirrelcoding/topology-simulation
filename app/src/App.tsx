	...
	return (
		<div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', margin: 0, padding: 0, overflow: 'hidden', background: 'black' }}>
			...
			<button
				...
				style={{ position: 'fixed', bottom: 30, right: 30, zIndex: 50, background: 'transparent', border: 'none', color: 'rgba(255,255,255,0.5)', fontSize: 48, fontWeight: 600, cursor: 'pointer', transition: 'color 0.3s ease', padding: 0 }}
				...
				<div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 200 }} onClick={() => setShowHelpModal(false)}>
					<div style={{ background: 'rgba(255,255,255,0.95)', borderRadius: 16, padding: '32px', maxWidth: 500, backdropFilter: 'blur(10px)', border: '1px solid rgba(255,255,255,0.3)', boxShadow: '0 8px 32px rgba(0,0,0,0.2)' }} onClick={(e) => e.stopPropagation()}>
						...
						<button
							...
							style={{ marginTop: 24, background: 'rgba(0,0,0,0.1)', border: 'none', padding: '8px 16px', borderRadius: 8, fontSize: 14, fontWeight: 500, cursor: 'pointer', transition: 'background 0.3s ease', color: 'black' }}
